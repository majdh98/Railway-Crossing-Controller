 ...
 *
 * Implemnts the FSM. Uses UART to configure the WIFI module, connect to WIFI,
 * and communicate with a substation server to poll the station status.
 * Uses led.h, gic.h, io.h, ttc.h, servo.h, and adc.h to control and LEDs,
...
#include "io.h"  		// Button and switches interface
#include "ttc.h"		// Processing system Triple Timer Counter interface
#include "servo.h"		// Servo module interface
#include "adc.h"		// Analog to Digital converter interface
...
// state machine and timers outputs
...
static void timer_callback(){
	...
	if (state != MAINTENANCE){
		...
		// read potentiometer value and set servo accordingly
		pot = adc_get_pot();
	...
	// check the server for any change
	update_request_t update_req = {2, 0, 1}; // id for our group
...
// change to incoming train state
...
int main(void) {
	...
	// initialize leds
	led_init();
	// starts at initial servo dutycycle
	servo_init();
